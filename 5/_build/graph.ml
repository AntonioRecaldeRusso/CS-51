...
struct
  ...
  print_string ((G.string_of_graph g)^"\n" ); 
  print_string ((G.string_of_graph g2)^"\n" );
  print_string ((string_of_int (List.length (G.nodes g)))^"\n" ) ;
  print_string ((string_of_int (List.length (G.nodes g2)))^"\n" ) 
  let _ = (
