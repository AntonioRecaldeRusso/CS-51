...
(** A White Walker will roam the world until it has destroyed a satisfactory
    number of towns *)
class white_walker (p : int * int) (kings_landing : KingsLanding.kings_landing) (wall) : Movable.movable_t =
object (self)
  ...
  method! draw = self#draw_circle (Graphics.rgb 0x89 0xCF 0xF0) Graphics.black (string_of_int vanquished_towns)
 ...
 (fun () -> Some (self.direction_from_to (self.pos) (World.size/2,World.size/2)))
 ( fun () -> Some (Direction.random World.rand))*)


  (* ### TODO: Part 6 Custom Events ### *)
  method private next_direction_default = with_inv_probability_or (World.rand) (World.size / 2 ) 
