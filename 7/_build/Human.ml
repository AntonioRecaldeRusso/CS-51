...
object(self)
  ...
  val mutable under_attack = false
  val mutable target = home#get_pos
  ...
  initializer
   ...
   self#register_handler (home#get_danger_event) (self#do_attack)
  ...
  (**** Human Methods ****)
  (***********************)
  method private attack obj = 
  if (self#get_pos = obj#get_pos) 
  then obj#receive_damage
  else ()
      
 

    (*let x = World.fold (fun x y-> if (x#get_name = "dragon") then x else y) (home) in
    let (a, b) = x#get_pos in  
  print_string (x#get_name ^ "at" ^ "(" ^ (string_of_int a) ^ "," ^ (string_of_int b) ^ ")\n") 
     *)
  method private do_attack =
  fun obj -> print_string "attack \n"; self#attack obj;
